import java.util.*;
class oconnor_EnhancedBankAccount extends oconnor_BankAccount
{
	...
	oconnor_EnhancedBankAccount(String ownerLastName, String ownerFirstName, double checkingBalance, double savingsBalance)
	{
	...
	oconnor_EnhancedBankAccount(String ownerLastName, String ownerFirstName)
	{
	...
	{
		ArrayList<Transaction> dateArray = new ArrayList<>();
		for(int i = 0; i < failedTransactions.size(); i++)
		{
			Date aDate = failedTransactions.get(i).date();
			if(aDate.compareTo(startDate) >= 0 && aDate.compareTo(endDate) <= 0)
			{
				dateArray.add(failedTransactions.get(i));
			}
		}
		return dateArray;
	...
	public java.util.ArrayList<Transaction> getSuccessfulTransactions(java.util.Date startDate, java.util.Date endDate)
	{
		...
		{
			Date aDate = successfulTransactions.get(i).date();
			if(aDate.compareTo(startDate) >= 0 && aDate.compareTo(endDate) <= 0)
			{
				dateArray.add(successfulTransactions.get(i));
			}
		}
		return dateArray;
		//return "Start Date:" + startDate + "End Date:" + endDate;
